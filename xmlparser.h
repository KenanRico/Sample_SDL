...
class XMLParser{
	...
	public:
		...
		static void TileMap(const char*, const char*, std::vector<TileLayer*>&, std::vector<ImageLayer*>&, std::vector<ObjectLayer*>&, std::vector<TileSet*>&, int&, int&, int&, int&, SDL_Renderer*, SDL_Window*);
	private:
		...
		static void parsetoobjectlayers(rapidxml::xml_node<>*, std::vector<ObjectLayer*>&);
	private:
